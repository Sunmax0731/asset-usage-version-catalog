...
- 作業ディレクトリ: `D:\AI\AssetPipeline\asset-usage-version-catalog`
...
```powershell
cd D:\AI\AssetPipeline\asset-usage-version-catalog
npm test
npm start
```
...
5. CLI + catalog JSON の UI または配布面を開き、検証結果をユーザーが次アクションに移せる粒度で確認する。
...
- docs ZIP が `dist/asset-usage-version-catalog-docs.zip` に存在する。
